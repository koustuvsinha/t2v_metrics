...
from .perceptionlm.apps.plm.generate import (
...
class PerceptionLMModel:
    ...
    def __init__(self, model_name="perception-lm-8b", device="cuda", cache_dir=None):
        assert model_name in PERCEPTION_LM_MODELS, (
            f"Model {model_name} not found in PERCEPTION_LM_MODELS"
        )
                ...
                if np_array.ndim == 3:  # Single image
                    ...
                    transform = get_image_transform(
                        vision_input_type=(
                            "vanilla"
                            if num_tiles == 1
                            else self.config.data.vision_input_type
                        ),
                    ...
                    frames_tensor, _ = transform(
                        (frames, len(frames), None, None, None)
                    )
    ...
    def forward(
        ...
        for data, question, answer in zip(processed_data, questions, answers):
            ...
            generator = PackedCausalTransformerGenerator(
                gen_cfg, self.model, self.tokenizer
            )
