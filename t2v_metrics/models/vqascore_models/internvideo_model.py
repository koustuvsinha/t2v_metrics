...
class InternVideo2Model(VQAScoreModel):
    ...
    def __init__(
        self, model_name="internvideo2-chat-8b", device="cuda", cache_dir=None
    ):
        assert model_name in INTERNVIDEO2_MODELS, (
            f"Model {model_name} not found in INTERNVIDEO2_MODELS"
        )
    ...
    def load_model(self):
        self.model = AutoModel.from_pretrained(**self.model_info["model"]).to(
            self.device
        )
    ...
    def load_video(self, video_path, num_segments=8, resolution=224, hd_num=6):
        if self.model_name == "internvideo2-chat-8b":
            ...
            return self.load_video_chat_hd(
                video_path, num_segments=8, return_msg=False, resolution=224, hd_num=6
            )
    ...
    def forward(
        ...
        for data, question, answer in zip(processed_data, questions, answers):
            ...
            chat_history = []
            media_type = (
                "video" if data.dim() == 6 else "image"
            )  # 6D for video, 5D for image
            instruction = None
            msg = ""
    ...
    def load_video_chat_hd(
        ...
        if padding:
            frames = self.HD_transform_padding(
                frames.float(), image_size=resolution, hd_num=hd_num
            )
        else:
            frames = self.HD_transform_no_padding(
                frames.float(), image_size=resolution, hd_num=hd_num
            )
    ...
    def HD_transform_no_padding(
        self, frames, image_size=224, hd_num=6, fix_ratio=(2, 1)
    ):
        min_num = 1
        max_num = hd_num
    ...
    def process_image_chat(self, image_path, resolution=224):
        ...
        d_hid = self.model.vision_encoder.encoder.img_pos_embed.shape[
            -1
    ...
    def process_image_chat_hd(
        self, image_path, resolution=224, hd_num=12, padding=False
    ):
        mean = (0.485, 0.456, 0.406)
        std = (0.229, 0.224, 0.225)
        ...
        if padding:
            ...
            img = self.HD_transform_no_padding(
                img, image_size=resolution, hd_num=hd_num
            )
    ...
    def get_sinusoid_encoding_table(
        self, n_position, d_hid, ckpt_num_frame=-1, cur_frame=12
    ):
    ...
    def generate_caption_chat_hd(
        ...
        text_embeds = self.model.pad_text_embeds(
            input_ids=input_ids,
            image=image,
            video=video,
            image_idx=image_idx,
